...
export const Menu = () => {
    ...
    const isBgPearl = bgUrl[path] === 'bg-devarana-pearl' ? true : false
    ...
    <>
        <div className={`fixed h-[60px] w-full items-center grid grid-cols-12 lg:justify-between lg:px-[138px] px-10 z-50 max-w-full transition-all duration-500 ease-in-out 
            ...
            <button onClick={() => onClickMenu(!activeMenu)} className='h-full px-2 z-[1001] col-span-2'><MenuSvg className={`${showHead === 1 || isBgPearl ? 'stroke-devarana-graph-darker' : 'stroke-white'}`} /></button>
            <Link href="/" className='col-span-6 col-start-4 mx-auto  hover:opacity-50 transition-all ease-in-out duration-500' onClick={ () => onClickMenu(false)}>
                <DevaranaInlineSvg className={`${showHead === 1 || isBgPearl ? 'fill-devarana-graph-darker' : 'fill-white'}  lg:w-[200px] w-[130px]`} />
            </Link>
            <div className='lg:flex items-center gap-10 hidden col-span-3 ml-auto'>
                <a className='hover:opacity-50 transition-all ease-in-out duration-500' target="_blank" rel="noreferrer" href='mailto:ventas@devarana.mx'>
                    <CorreoSvg className={`${showHead === 1 || isBgPearl ? 'fill-devarana-graph-darker' : 'fill-white'} w-[25px]`} />
                </a>
                <a className='hover:opacity-50 transition-all ease-in-out duration-500' target="_blank" rel="noreferrer" href='https://web.whatsapp.com/send?phone=+524428244444&text=Hola, quisiera más información de Devarana.'>
                    <WhatsappSvg className={`${showHead === 1 || isBgPearl ? 'fill-devarana-graph-darker' : 'fill-white'} w-[25px]`} />
                </a>
                <a href='tel:+524428244444' className={`${showHead === 1 || isBgPearl ? 'text-devarana-graph-darker' : 'text-white'} font-light text-base hover:opacity-50 transition-all ease-in-out duration-300 tracking-wider`}>(442) 824 4444</a>
            </div>
